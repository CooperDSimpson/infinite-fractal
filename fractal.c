// Progressive Mandelbrot - fully working version
#include <windows.h>
#include "glad.h"
#include <stdio.h>
#include <stdlib.h>
...
dd_t dd_add(dd_t a, dd_t b){ double s=a.hi+b.hi; double v=s-a.hi; double t=((b.hi-v)+(a.hi-(s-v)))+a.lo+b.lo; return (dd_t){s+t,t-(s+t-s)}; }
dd_t dd_sub(dd_t a, dd_t b){ double s=a.hi-b.hi; double v=s-a.hi; double t=(( -b.hi-v)+(a.hi-(s-v)))+a.lo-b.lo; return (dd_t){s+t,t-(s+t-s)}; }
dd_t dd_mul(dd_t a, dd_t b){ double p=a.hi*b.hi; double e=fma(a.hi,b.hi,-p)+a.hi*b.lo+a.lo*b.hi+a.lo*b.lo; return (dd_t){p+e,e-(p+e-p)}; }
...
// Fragment shader renders Mandelbrot iteration
...
dd dd_add(dd a, dd b){ double s=a.hi+b.hi; double v=s-a.hi; double t=((b.hi-v)+(a.hi-(s-v)))+a.lo+b.lo; return dd(s+t,t-(s+t-s)); }
dd dd_sub(dd a, dd b){ double s=a.hi-b.hi; double v=s-a.hi; double t=(( -b.hi-v)+(a.hi-(s-v)))+a.lo-b.lo; return dd(s+t,t-(s+t-s)); }
dd dd_mul(dd a, dd b){ double p=a.hi*b.hi; double e=fma(a.hi,b.hi,-p)+a.hi*b.lo+a.lo*b.hi+a.lo*b.lo; return dd(p+e,e-(p+e-p)); }
dd dd_from_double(double x){ return dd(x,0.0); }

void main(){
    dd cx = dd_add(dd_from_double(u_center.x), dd_mul(dd_from_double(uv.x*2-1), dd_from_double(u_scale)));
    dd cy = dd_add(dd_from_double(u_center.y), dd_mul(dd_from_double(uv.y*2-1), dd_from_double(u_scale)));
    dd zx = dd_from_double(0.0); dd zy=dd_from_double(0.0);

    int iter=0;
    for(int i=0;i<u_maxIter;i++){
        dd zx2=dd_mul(zx,zx); dd zy2=dd_mul(zy,zy); dd zxzy=dd_mul(zx,zy);
    ...
    vec3 color = vec3(0.3,0.0,0.6)*t*(0.5+0.5*sin(3.1415*t*10.0));
...
int main(){
    ...
    RegisterClassA(&wc);
    HWND hwnd=CreateWindowA("FractalWindow","Mandelbrot",WS_OVERLAPPEDWINDOW|WS_VISIBLE,100,100,width,height,NULL,NULL,wc.hInstance,NULL);
